...
def digitalReader(Img):
    ...
    for d in digit:
        ...
        #print(roi[0])
        x=x+w-w_max
        dW=int(.20*roi.shape[0])
        #print(dW)
        dH=int(0.15*roi.shape[1])
        breakout=[  
            ((x,y),(x+w_max,y+dH)), #top
        ...
        for (e,((x1,y1),(x2,y2))) in enumerate(breakout):
                #print(e)
                #print(((x1,y1),(x2,y2)))
                cv2.rectangle(clean,(x1,y1),(x2,y2),(255,0,0),2)
...
def sqFinder(Img):
    ...
    warped=im.resize(warped, width=400)
    val=[warped,edged]
...
def main():
    gaugeImg = cv2.imread('C:\\Users\\\OneDrive\\Desktop\\ME_459\\Project\\IMG_3672.JPEG')
    digitalImg = cv2.imread('C:\\Users\\\OneDrive\\Desktop\\ME_459\\FinalProject\\ME459FinalProject\\testing.JPEG')
